...
@NamedQueries({
	...
	@NamedQuery(name="Movie.findByActorBetween", query="SELECT movie FROM Movie movie JOIN movie.roles role JOIN role.actor actor WHERE actor.id = :actorId AND movie.releaseDate >= :startYear AND movie.releaseDate <= :endYear ORDER BY movie.releaseDate"),
